...
const SERVICE_CATEGORIES = new Set(["Agência veicular", "Autopeças", "Oficina", "Outras"]);
...
function validate(data) {
  const fields = ["fullName", "cnpj", "email", "phone", "businessName", "region", "category", "description"];
  ...
  if (!isValidCnpj(data.cnpj)) return "Informe um CNPJ válido. Cadastros de empresa não aceitam CPF.";
  if (String(data.phone).replace(/\D/g, "").length < 10) return "Informe telefone ou WhatsApp válido.";
  if (!SERVICE_CATEGORIES.has(String(data.category).trim())) return "Selecione uma categoria de serviço válida.";
  if (String(data.description).trim().length < 10) return "Descreva brevemente os serviços da empresa.";
...
async function saveRegistration(data) {
  ...
  const payload = {
    full_name: data.fullName.trim(), cpf_cnpj: normalizeCnpj(data.cnpj), document_type: "cnpj", email: data.email.trim(), phone: data.phone.trim(),
  ...
  let failureText = "";
  if (!result.ok) failureText = await result.text();
  if (!result.ok && payload.cep && /\bcep\b|schema cache|PGRST204/i.test(failureText)) {
  ...
  if (!result.ok) throw new Error(`Supabase respondeu ${result.status}: ${failureText || await result.text()}`);
...
if (process.argv[1] && fileURLToPath(import.meta.url) === normalize(process.argv[1])) {
  server.listen(PORT, () => console.log(`Cadastro disponível em http://localhost:${PORT}`));
}

export { SERVICE_CATEGORIES, isValidCnpj, normalizeCnpj, validate, server, requestHandler };
