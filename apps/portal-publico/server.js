...
function isPublicCompany(item) {
  const cnpj = String(item?.cpf_cnpj || "").replace(/\D/g, "");
  return item?.document_type === "cnpj" && /^\d{14}$/.test(cnpj) && !/^(\d)\1{13}$/.test(cnpj) && SERVICE_CATEGORIES.includes(String(item?.category || "").trim());
}

async function getActiveCompanies() {
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) return [];
  const headers = { apikey: SUPABASE_SECRET_KEY, Authorization: `Bearer ${SUPABASE_SECRET_KEY}` };
  const baseFields = 'id,business_name,region,category,description,public_description,days_hours,booth_location,photo_url,logo_url,public_ready,is_featured,created_at,status,cpf_cnpj,document_type';
  ...
  if (!response.ok) throw new Error(`Supabase Empresas indisponível (${response.status}). Execute a migração de empresas.`);
  return (await response.json()).filter(isPublicCompany).map((item) => ({
    id: item.id,
...
async function getFeaturedCompanies() {
  return (await getActiveCompanies()).filter((company) => company.isFeatured).slice(0, 3);
}
...
function isTestDriveVideo(video) {
  return /#test[-\s]?drive\b/i.test(`${video.title || ''} ${video.fullDescription || video.description || ''}`);
}
  ...
  if (YOUTUBE_API_KEY) {
    ...
    do {
      ...
      for (const item of data.items || []) {
        ...
        videos.push({ id, title: snippet.title, description: resumir(snippet.description || ''), fullDescription: snippet.description || '', thumbnail: snippet.thumbnails?.high?.url || snippet.thumbnails?.medium?.url || snippet.thumbnails?.default?.url || `https://i.ytimg.com/vi/${id}/hqdefault.jpg`, publishedAt: snippet.publishedAt || '', url: `https://www.youtube.com/watch?v=${id}` });
      }
...
async function getPlaylistVideosByType(playlistId, type) {
...
http.createServer(async (request, response) => {
  if (request.url?.startsWith('/api/companies-featured')) {
    try {
      ...
      sendJson(response, 502, { error: 'Não foi possível atualizar as Empresas em Destaque.', details: error.message });
    }
    return;
  }
  ...
  if (request.url?.startsWith('/api/companies')) {
    try {
      ...
      sendJson(response, 502, { error: 'Não foi possível atualizar as empresas.', details: error.message });
    }
    return;
  }
