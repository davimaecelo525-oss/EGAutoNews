...
function renderEmpresasDestaque(companies) {
  const grid = document.querySelector('#empresasDestaqueGrid');
  if (!grid) return;
  if (!Array.isArray(companies) || !companies.length) {
    grid.innerHTML = '<p class="feiras-destaque-estado">Nenhuma empresa em destaque no momento.</p>';
    return;
  }
  grid.innerHTML = companies.slice(0, 3).map((company) => {
    const image = escapeHtml(company.photoUrl || 'images/foto.avif');
    const name = escapeHtml(company.businessName || 'Empresa em destaque');
    const description = escapeHtml(company.description || 'Conheça os serviços desta empresa parceira.');
    const place = [company.region, company.cep ? `CEP ${formatarCepHome(company.cep)}` : ''].filter(Boolean).join(' · ');
    const target = `feirantes/feirante.html?id=${encodeURIComponent(company.id)}`;
    return `<a class="card-destaque card-clicavel" href="${target}" aria-label="Ver detalhes de ${name}"><div class="img-destaque"><img src="${image}" alt="${name}" loading="lazy"><span class="selo-feira">${escapeHtml(company.category || 'EMPRESA EM DESTAQUE')}</span></div><div class="conteudo-destaque"><h3>${name}</h3><p class="descricao-feira">${description}</p><p class="localizacao-feira">⌖ ${escapeHtml(place || 'Distrito Federal')}</p></div></a>`;
  }).join('');
}

async function atualizarEmpresasDestaque() {
  const grid = document.querySelector('#empresasDestaqueGrid');
  if (!grid) return;
  try {
    const response = await fetch('/api/companies-featured', { cache: 'no-store' });
    if (!response.ok) throw new Error('Empresas em destaque indisponíveis.');
    const { companies = [] } = await response.json();
    renderEmpresasDestaque(companies);
  } catch (error) {
    console.warn('Empresas em destaque: não foi possível carregar os dados.', error);
    renderEmpresasDestaque([]);
  }
}

document.addEventListener('DOMContentLoaded', () => {
  atualizarEmpresasDestaque();
  const grupos = [
...
function aplicarNoticiaHome(card, noticia) {
  if (!card || !noticia) return;
  const imagemPadrao = 'images/foto.avif';
  const destino = noticia.externalUrl || `noticias/index.html?slug=${encodeURIComponent(noticia.slug || '')}`;
  card.dataset.url = destino;
  ...
  if (titulo) titulo.textContent = noticia.title || 'Última notícia';
  if (descricao) descricao.textContent = noticia.excerpt || noticia.content || 'Leia a notícia completa.';
  if (imagem) {
    imagem.onerror = () => { if (!imagem.src.endsWith(imagemPadrao)) imagem.src = imagemPadrao; };
    imagem.src = noticia.imageUrl || imagemPadrao;
    imagem.alt = noticia.title || 'Notícia';
  }
  if (selo) selo.textContent = noticia.source === 'motor1' ? 'MOTOR1' : 'NOTÍCIA DO PORTAL';
  if (link) {
    link.href = destino;
    link.target = noticia.externalUrl ? '_blank' : '_self';
    link.rel = noticia.externalUrl ? 'noopener noreferrer' : '';
    link.textContent = noticia.externalUrl ? 'Ler no Motor1 →' : 'Leia mais →';
  }
}

async function atualizarNoticiasHome() {
  const cards = [...document.querySelectorAll('#noticias .card-noticia')].slice(0, 3);
  if (!cards.length) return;
  try {
    const response = await fetch('/api/news', { cache: 'no-store' });
    if (!response.ok) throw new Error('Últimas notícias indisponíveis.');
    const { news = [] } = await response.json();
    if (!news.length) throw new Error('Nenhuma notícia disponível.');
    news.slice(0, cards.length).forEach((noticia, index) => aplicarNoticiaHome(cards[index], noticia));
  } catch (error) { console.warn('Últimas notícias: não foi possível atualizar os cards.', error); }
}

atualizarNoticiasHome();
window.setInterval(atualizarNoticiasHome, 15 * 60 * 1000);
