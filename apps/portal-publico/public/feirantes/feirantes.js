...
function scheduleHtml(value) {
  ...
  return `<div class="feirante-horarios-list">${rows.map((row) => {
    ...
    if (separator < 0) return `<div class="feirante-horario-item"><strong class="feirante-horario-periodo">${escapeHtml(row)}</strong></div>`;
...
function cardHtml(feirante) {
  const title = feirante.businessName || 'Empresa';
...
async function renderDetail(feirante) {
  const title = feirante.businessName || 'Empresa';
  document.title = `${title} | EGAutoNews`;
...
function render() {
  ...
  $('#feirantesResumo').textContent = `${visible.length} empresa${visible.length === 1 ? '' : 's'} encontrada${visible.length === 1 ? '' : 's'}`;
  const parts = [];
...
async function loadFeirantes() {
  try {
    ...
    if (!response.ok) throw new Error('Empresas indisponíveis');
    const data = await response.json();
    state.feirantes = Array.isArray(data.companies) ? data.companies : [];
    ...
    if (!$('#feirantesResumo') || !$('#feirantesGrid')) return;
    try {
    ...
    render();
      ...
      console.warn('Empresas públicas:', error);
...
function playlistVideoCard(video) {
  ...
  const description = escapeHtml(video.description || 'Confira este vídeo da EG Auto News.');
  const thumbnail = escapeHtml(video.thumbnail || '');
  const url = escapeHtml(video.url || '#');
  return `<article class="feirantes-video-card"><a href="${url}" target="_blank" rel="noopener noreferrer" class="feirantes-video-thumb"><img src="${thumbnail}" alt="${title}" loading="lazy"><span class="feirantes-video-play" aria-hidden="true">▶</span></a><div class="feirantes-video-card-body"><h3>${title}</h3><p>${description}</p><a class="feirantes-video-link" href="${url}" target="_blank" rel="noopener noreferrer">Assistir vídeo →</a></div></article>`;
}
async function loadFeirantesPlaylists() {
  const videosGrid = $('#feirantesVideosGrid');
  const testDriveGrid = $('#feirantesTestDriveGrid');
  if (!videosGrid || !testDriveGrid) return;
  try {
    const response = await fetch('/api/eg-auto-news', { cache: 'no-store' });
    if (!response.ok) throw new Error('Playlist indisponível');
    const data = await response.json();
    const videos = Array.isArray(data.videos) ? data.videos : [];
    videosGrid.innerHTML = videos.length ? videos.map(playlistVideoCard).join('') : '<p class="feirantes-video-estado">Nenhum vídeo disponível nesta playlist.</p>';
  } catch {
    videosGrid.innerHTML = '<p class="feirantes-video-estado">Não foi possível carregar os vídeos agora.</p>';
  }
  try {
    const response = await fetch('/api/test-drive', { cache: 'no-store' });
    const data = response.ok ? await response.json() : { videos: [], configured: false };
    const videos = Array.isArray(data.videos) ? data.videos : [];
    testDriveGrid.innerHTML = videos.length ? videos.map(playlistVideoCard).join('') : '<p class="feirantes-video-estado">A playlist de Test Drive será adicionada aqui.</p>';
  } catch {
    testDriveGrid.innerHTML = '<p class="feirantes-video-estado">A playlist de Test Drive será adicionada aqui.</p>';
  }
}
loadFeirantesPlaylists();
