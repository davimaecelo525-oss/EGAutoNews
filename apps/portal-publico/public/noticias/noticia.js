...
function articleRow(article) {
  const href = article.externalUrl ? article.externalUrl : `index.html?slug=${encodeURIComponent(article.slug)}`;
  ...
  return `<article class="noticia-linha"><img class="noticia-miniatura" src="${image}" alt="${title}" onerror="this.onerror=null;this.src='${fallbackImage}'"><div class="noticia-linha-conteudo"><h2>${title}</h2><p class="noticia-linha-data">${dates}</p><a class="noticia-linha-botao" href="${href}"${article.externalUrl ? ' target="_blank" rel="noopener noreferrer"' : ''}>Ler mais...</a></div></article>`;
}
...
async function loadArticle(slug) {
  try {
    ...
    if (article.externalUrl) { window.location.assign(article.externalUrl); return; }
