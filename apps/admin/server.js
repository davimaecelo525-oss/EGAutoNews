...
function requireAuth(req, res, next) {
  ...
  if (!token || !sessions.has(token)) return sendError(res, 401, "Faça login para acessar o painel.");
...
function mapRegistration(row) {
  return {
    ...
    documentType: row.document_type || "legacy",
    email: row.email,
    phone: row.phone,
...
const SERVICE_CATEGORIES = ["Agência veicular", "Autopeças", "Oficina", "Outras"];

function isValidCnpj(value) {
  const digits = String(value || "").replace(/\D/g, "");
  return /^\d{14}$/.test(digits) && !/^(\d)\1{13}$/.test(digits);
}

function isCompanyRecord(row) {
  return row?.document_type === "cnpj" && isValidCnpj(row?.cpf_cnpj) && SERVICE_CATEGORIES.includes(String(row?.category || "").trim());
}
...
async function assertFeaturedCompanyCapacity({ id = null, isFeatured }) {
  if (!isFeatured) return;
  const rows = await supabaseRequest('stall_registrations?select=id&status=eq.approved&public_ready=eq.true&is_featured=eq.true');
  const occupied = rows.filter((row) => String(row.id) !== String(id)).length;
  if (occupied >= 3) throw new Error("Você pode manter no máximo três Empresas em Destaque na Home.");
}
...
app.get("/api/me", (req, res) => {
  const token = getSessionToken(req);
  const admin = token ? sessions.get(token) : null;
...
app.get("/api/companies", requireAuth, async (req, res) => {
  try {
    const rows = await supabaseRequest("stall_registrations?select=*&order=business_name.asc,created_at.desc");
    const query = String(req.query.q || "").trim().toLowerCase();
    const status = String(req.query.status || "all");
    const category = String(req.query.category || "all");
    const companies = rows.filter((row) => row.status === "approved" && isCompanyRecord(row)).map(mapRegistration).map((record) => ({
      ...record,
      status: record.publicReady ? "active" : "inactive"
    })).filter((item) => {
      const searchable = [item.name, item.businessName, item.category, item.region, item.document].join(" ").toLowerCase();
      return (status === "all" || item.status === status) && (category === "all" || item.category === category) && (!query || searchable.includes(query));
    });
    res.json({ companies, total: companies.length, categories: SERVICE_CATEGORIES });
  } catch (error) {
    console.error(error);
    sendError(res, 502, error.message);
  }
...
app.patch("/api/registrations/:id/profile", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!isCompanyRecord(current)) return sendError(res, 400, "Este cadastro não possui CNPJ válido. Execute a migração de empresas ou conclua um novo cadastro.");
    ...
    if (!SERVICE_CATEGORIES.includes(category)) return sendError(res, 400, "Selecione uma categoria de serviço válida.");
...
app.patch("/api/companies/:id/status", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  const status = String(req.body?.status || "");
  if (!Number.isSafeInteger(id) || !["active", "inactive"].includes(status)) return sendError(res, 400, "ID ou status da empresa inválido.");
  try {
    const currentRows = await supabaseRequest(`stall_registrations?id=eq.${id}&select=*`);
    const current = currentRows[0];
    if (!current || current.status !== "approved" || !isCompanyRecord(current)) return sendError(res, 404, "Empresa aprovada não encontrada.");
    if (status === "active" && !isPublicProfileComplete(current)) return sendError(res, 400, "Complete empresa, região, categoria, descrição, foto e logo antes de ativar.");
    const rows = await supabaseRequest(`stall_registrations?id=eq.${id}`, { method: "PATCH", body: JSON.stringify({ public_ready: status === "active", updated_at: new Date().toISOString() }) });
    if (!rows.length) return sendError(res, 404, "Empresa não encontrada.");
    res.json({ record: { ...mapRegistration(rows[0]), status } });
  } catch (error) { console.error(error); sendError(res, 502, error.message); }
});
...
app.delete("/api/companies/:id", requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isSafeInteger(id)) return sendError(res, 400, "ID da empresa inválido.");
  try {
    const currentRows = await supabaseRequest(`stall_registrations?id=eq.${id}&select=*`);
    const current = currentRows[0];
    if (!current || current.status !== "approved" || !isCompanyRecord(current)) return sendError(res, 400, "Somente empresas aprovadas podem ser excluídas nesta área.");
    await supabaseRequest(`stall_registrations?id=eq.${id}`, { method: "DELETE", headers: { Prefer: "return=minimal" } });
    res.json({ ok: true });
  } catch (error) { console.error(error); sendError(res, 502, error.message); }
});
