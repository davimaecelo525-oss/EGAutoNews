...
async function api(url, options = {}) {
  ...
  if (!response.ok) {
    if (response.status === 404 && url.startsWith("/api/companies/")) throw new Error("Servidor Admin desatualizado: reinicie o painel para carregar as rotas de Empresas.");
...
function tableMarkup(records, compact = false) {
  ...
  return `<div class="table-scroll"><table><thead><tr><th>Responsável / empresa</th>${compact ? "" : "<th>CNPJ</th><th>Telefone</th>"}<th>Categoria</th><th>Status</th><th>Recebido em</th>${compact ? "" : "<th>Ação</th>"}</tr></thead><tbody>${records.map((record) => recordRow(record, compact)).join("")}</tbody></table></div>`;
}

function feiranteRow(record) {
  return `<tr>
    <td><strong>${escapeHtml(record.businessName || "—")}</strong><small>${escapeHtml(record.name || "Responsável não informado")}</small></td>
    <td>${escapeHtml(record.category || "—")}</td>
    ...
    <td class="actions-cell"><button class="details-button" type="button" data-details-id="${record.id}">Ver detalhes</button><button class="details-button" type="button" data-edit-stall="${record.id}">Editar dados</button><select class="feirante-status-select" data-id="${record.id}" aria-label="Ativar ou inativar empresa"><option value="active" ${record.status === "active" ? "selected" : ""}>Ativa</option><option value="inactive" ${record.status === "inactive" ? "selected" : ""}>Inativa</option></select><button class="text-danger" type="button" data-delete-feirante="${record.id}">Excluir</button></td>
  </tr>`;
}

function feirantesTableMarkup(records) {
  if (!records.length) return `<div class="empty"><strong>Nenhuma empresa aprovada encontrada</strong><span>Cadastros em análise e recusados ficam em Cadastros recebidos. Empresas aprovadas aparecem aqui, separadas por categoria.</span></div>`;
  return `<div class="table-scroll"><table><thead><tr><th>Empresa / responsável</th><th>Categoria</th><th>Região</th><th>Horários</th><th>Status</th><th>Atualizado em</th><th>Ações</th></tr></thead><tbody>${records.map(feiranteRow).join("")}</tbody></table></div>`;
}
...
async function loadFeirantes() {
  ...
  const category = encodeURIComponent($("#feirantesCategory").value);
  $("#feirantesTable").innerHTML = `<div class="loading">Carregando empresas...</div>`;
  const data = await api(`/api/companies?q=${query}&status=${status}&category=${category}`);
  state.records = data.companies;
  $("#feirantesTable").innerHTML = feirantesTableMarkup(data.companies);
...
function openDetails(record) {
  $("#detailsTitle").textContent = record.businessName || record.name || "Detalhes da empresa";
  ...
  $("#detailsContent").innerHTML = `<div class="details-grid">
    ...
    ${detailField("Empresa", record.businessName)}
    ${detailField("CNPJ", formatDocument(record.document))}
...
function openStallModal(record) {
  ...
  form.elements.isFeatured.checked = record?.isFeatured === true;
...
async function saveStall(event) {
  ...
  try {
    ...
    closeStallModal(); showToast("Dados da empresa atualizados com sucesso.");
    await refreshAll();
...
function bindFeiranteStatusSelects() {
  document.querySelectorAll(".feirante-status-select").forEach((select) => select.addEventListener("change", async () => {
    ...
    select.disabled = true;
    try {
      await api(`/api/companies/${id}/status`, { method: "PATCH", body: JSON.stringify({ status: select.value }) });
      showToast(select.value === "active" ? "Empresa ativada no portal." : "Empresa inativada.");
      await refreshAll();
      if (state.section === "feirantes") await loadFeirantes();
    } catch (error) { showToast(error.message, true); select.disabled = false; }
  }));
...
function bindDeleteFeiranteButtons() {
  document.querySelectorAll("[data-delete-feirante]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (!record || !window.confirm(`Excluir definitivamente a empresa "${record.businessName || record.name}"?`)) return;
    button.disabled = true;
    try {
      await api(`/api/companies/${id}`, { method: "DELETE" });
      showToast("Empresa excluída do portal.");
      await refreshAll();
      if (state.section === "feirantes") await loadFeirantes();
...
function sponsorRow(sponsor) {
  const placementLabels = { home_bottom: "Home", home_split: "Home — grade", home_top: "Topo", feirantes_inline: "Feirantes", news_sidebar: "Notícias", news_sidebar_top: "Notícias — lateral superior", news_sidebar_bottom: "Notícias — lateral inferior", external_left_500: "Páginas externas — esquerdo 500x250", external_left_250: "Páginas externas — esquerdo 250x250", external_right_500: "Páginas externas — direito 500x250", external_right_250: "Páginas externas — direito 250x250", all: "Todas" };
...
function showSection(section) {
  ...
  $("#sectionTitle").textContent = section === "overview" ? "Visão geral" : section === "feirantes" ? "Empresas" : section === "fairs" ? "Feiras cadastradas" : section === "sponsors" ? "Patrocinadores" : section === "news" ? "Notícias" : "Cadastros recebidos";
...
["#feirantesStatus", "#feirantesCategory", "#registrationsStatus", "#fairsStatus", "#sponsorsStatus"].forEach((id) => $(id).addEventListener("change", (event) => event.currentTarget.closest("section").querySelector(".button.outline").click()));
